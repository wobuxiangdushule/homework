...
class GA(object):
    ...
    def ga_cross(self, x, y, crossover_prob):
        len_ = len(x)
        assert len(x) == len(y)
        ...
        order = list(np.random.choice(path_list, 2, replace=False))
        order.sort()
        start, end = order
    ...
    def ga(self):
        ...
        while len(fruits) < self.num_total:
            ...
            # 交叉
            gene_x_new, gene_y_new = self.ga_cross(gene_x, gene_y,self.crossover_prob)
            # 变异
    ...
    def run(self):
        ...
        f = open('test.txt', 'a')
        f.write(f"run end: {1./best_score}\n ")
        f.close()
...
def read_tsp(path):
    ...
    fileHandle.readline()  # ignore the first line
    city_number = int(fileHandle.readline())
    # print city_number
    best_known_value = int(fileHandle.readline())
    ...
    for line in data:
        ...
        tmp.append(tmpline)
    data = tmp
    return data,best_known_value,city_number

for i in range(1,7):
    if i==1:
        data,best,city_num = read_tsp(r'D:\ai！！！！\mytsp\TSPLIB6small\01eil51.txt')
    elif i==2:
        data, best, city_num = read_tsp(r'D:\ai！！！！\mytsp\TSPLIB6small\02eil76.txt')
    elif i==3:
        data, best, city_num = read_tsp(r'D:\ai！！！！\mytsp\TSPLIB6small\03eil101.txt')
    elif i==4:
        data, best, city_num = read_tsp(r'D:\ai！！！！\mytsp\TSPLIB6small\04pr107.txt')
    elif i==5:
        data, best, city_num = read_tsp(r'D:\ai！！！！\mytsp\TSPLIB6small\05bier127.txt')
    elif i==6:
        data, best, city_num = read_tsp(r'D:\ai！！！！\mytsp\TSPLIB6small\06pr136.txt')
    else:
        print("warring")
    ...
    model = GA(num_city=data.shape[0], num_total=100, iteration=500, data=data.copy())
    ...
    axs[0].set_title('last end')
    iterations = range(model.iteration)
    best_record = model.best_record
    axs[1].plot(iterations, best_record)
    axs[1].set_title('Convergence curve',y=-0.25)
    f = open('test.txt', 'a')
    f.write(f"cith number is {city_num},best is {best}\n\n ")
    f.close()
    mar = plt.get_current_fig_manager()
    mar.window.showMaximized()
    plt.savefig(fr"D:\ai！！！！\TSP_collection-master\TSP_collection-master\pictures\2+1+{i}",bbox_inches = 'tight')
    # plt.show()
    plt.close()
